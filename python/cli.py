...
Historical & Pipelines:
  ...
  bandar         Compute bandarmology accumulation/distribution scores
...
def main():
    ...
    if cmd == "company":
        ...
        date_arg = sys.argv[2] if len(sys.argv) > 2 else None
        print(f"=== Computing Bandarmology Scores ({date_arg or 'today'}) ===")
        scores = run_bandarmology(target_date=date_arg)
        top = scores[:10]
        if top:
            print(f"{'Code':<7}{'Score':<8}{'Chg%':<8}{'VolZ':<7}{'FreqZ':<7}{'FNetRatio':<10}")
            for s in top:
                print(f"{s['StockCode']:<7}{s['AccumulationScore']:<8}{s['PriceChangePct']:<8}{s['VolumeZScore']:<7}{s['FrequencyZScore']:<7}{s['ForeignNetRatio']:<10}")
        else:
            print("No scores computed (insufficient history — run `backfill` first).")
    elif cmd == "all":
