...
def ingest_daily(date=None, client=None, export_parquet=True):
    ...
    # ── Corporate Actions (15 categories: HMETD, stock split, buyback, IPO, etc.) ─
    try:
        from idx.scrapers.corporate import fetch_corporate_actions
        ca_result = fetch_corporate_actions(client=client)
        results["corporate_actions"] = {"status": "ok", "total_records": ca_result.get("totalRecordsAllTypes", 0)}
        log.info("Corporate actions refreshed: %d records", ca_result.get("totalRecordsAllTypes", 0))
    except Exception as exc:
        log.error("Corporate actions fetch failed: %s", exc)
        results["corporate_actions"] = {"status": "error", "message": str(exc)}

    # ── Market News & Headlines ─────────────────────────────────────────
    try:
        from idx.scrapers.news import fetch_news_search
        from idx.core.utils import save_json
        news_data = fetch_news_search(page_number=1, page_size=100, client=client)
        if news_data:
            news_file = os.path.join(DATA_DIR, "news_latest.json")
            save_json(news_file, news_data)
            n_items = len(news_data.get("data", [])) if isinstance(news_data, dict) else 0
            results["news"] = {"status": "ok", "items": n_items}
            log.info("Market news refreshed: %d headlines", n_items)
        else:
            results["news"] = {"status": "empty"}
    except Exception as exc:
        log.error("News fetch failed: %s", exc)
        results["news"] = {"status": "error", "message": str(exc)}
    ...
    # ── Bandarmology Scoring ─────────────────────────────────────────────
    try:
        from idx.pipelines.bandarmology import run as run_bandarmology
        scores = run_bandarmology(target_date=date_iso)
        results["bandarmology"] = {"status": "ok", "scored": len(scores)}
        log.info("Bandarmology scoring complete: %d stocks scored", len(scores))
    except Exception as exc:
        log.error("Bandarmology scoring failed: %s", exc)
        results["bandarmology"] = {"status": "error", "message": str(exc)}

    # ── Website Data Bundle ──────────────────────────────────────────────
    try:
        from idx.pipelines.website import build_website_bundle
        build_website_bundle(date_iso)
        results["website_bundle"] = {"status": "ok"}
        log.info("Website data bundle refreshed")
    except Exception as exc:
        log.error("Website bundle build failed: %s", exc)
        results["website_bundle"] = {"status": "error", "message": str(exc)}
